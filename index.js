  ...
  client.on("ready", async () => {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        channel.send(`<@716390085896962058> m add ${filteredPokemon[0]} 1000`);
        channel
          .createMessageCollector({
            filter: (msg) =>
              msg.author.id === "716390085896962058" &&
            ...
            channel
              ...
              .on("collect", async (msg) => {
                ...
                if (listingIdMatch) {
                  ...
                  sendLog(
                    client.user.username,
                    `Listed pokemon for sale!`,
                    "success"
                  );
                } else {
                  sendLog(
                    client.user.username,
                    "Listing ID not found in the message.",
                    "error"
                  );
                }
              });
      ...
      if (tasks[client.user.username].spend > 0) {
        ...
        if (availableListings.length > 0) {
          const randomIndex = Math.floor(
            Math.random() * availableListings.length
          );
          listingId = availableListings[randomIndex];
        } else {
          sendLog(
            client.user.username,
        ...
        if (listingId) {
          channel.send(`<@716390085896962058> m buy ${listingId}`);
          channel
            .createMessageCollector({
              filter: (msg) =>
                msg.author.id === "716390085896962058" &&
